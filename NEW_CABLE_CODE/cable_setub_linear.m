...
V_1=Ca_to_AP(A_Ca_prev(iii),phi,p,kk);
...
Ca_B(iii)= AP_to_Ca(mean(psi_h(end,1:end/3))*1e3,phi,p,kk);
end

error = mse(Ca_B,A_Ca_next');
% clear Ca_B
 % end
% figure
...
% my_p = find(error==min(error));
 ...
 figure
  plot([1:length(Ca_B)],Ca_B,'r--o','LineWidth',2,'MarkerSize',15)
  hold on
  plot([1:length(Ca_B)],A_Ca_next,'b--o','LineWidth',2,'MarkerSize',15)
